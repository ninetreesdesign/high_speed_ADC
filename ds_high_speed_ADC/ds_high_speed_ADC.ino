...
// these includes were substantially revised in 2020; latest code at https://github.com/pedvide/ADC
#include <ADC.h>
#include <ADC_util.h>
...
// Arrays to hold sample data in ram
...
void loop() {
    ...
    if (first_loop_flag == 1) {
        if (sample_timer >= (TIME_STEP_US-LOOP_OVERHEAD) && index < A_SIZE) {
            ...
            sample_timer = 0; //-= TIME_STEP_US;        // reset for either fixed interval or fixed minimal start interval
			// function needs to be called continuously, so is inside main loop
